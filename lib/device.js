...
class Device extends Homey.Device {
  onInit() {
    ...
    this.registerCapabilityListener('onoff', (value, opts) => {
      ...
      this.controlToggleX('0', (value ? 1 : 0), (err, res) => {
    ...
    this.getSystemAllData((err, deviceAllData) => {
      ...
      this.setValuesToggleX(this.uuid, deviceAllData.all.digest);
    });
  ...
  setValuesToggleX(deviceId, payload) {
    ...
    if (payload && payload.togglex) {
      ...
      payload.togglex.forEach((val) => {
        ...
        //console.log("Set new State: Channel - " + val.channel + " State : " + val.onoff)
        this.setCapabilityValue('onoff', val.onoff);
        //console.log("Capabilities: " + this.getCapabilityValue('onoff'));
      });
