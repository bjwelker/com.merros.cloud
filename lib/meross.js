...
class Meross extends EventEmitter {
    ...
    async connect({ retry = true, retries = null, retryInterval = 5000, UUID, callback } = {}) {
        ...
        while (true) {
            try {
                ...
                this.client.on('offline', async () => {
                    ...
                    await this.connect({ retry, initialRetries, retryInterval, UUID});
                });
    ...
    _connect() {
        ...
        console.log(this.uuid + "-" + this.userId + "-" + this.key); 
